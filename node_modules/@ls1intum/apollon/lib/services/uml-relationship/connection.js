...
var Connection = /** @class */ (function () {
    ...
    Connection.computePath = function (source, target, options) {
        var sourcePortPosition = source.element.ports()[source.direction].add(source.element.bounds.x, source.element.bounds.y);
        var targetPortPosition = target.element.ports()[target.direction].add(target.element.bounds.x, target.element.bounds.y);
        ...
        var path = tslib_1.__spread(pathFromStart, pathToEnd);
    ...
    Connection.tryFindStraightPath = function (source, target) {
        ...
        /*
            ...
            #######           #######
        */
        if (source.direction === uml_element_port_1.Direction.Right &&
            target.direction === uml_element_port_1.Direction.Left &&
        ...
        /*
            ...
            #######           #######
        */
        if (source.direction === uml_element_port_1.Direction.Left &&
            target.direction === uml_element_port_1.Direction.Right &&
        ...
        /*
            ...
            #######
        */
        if (source.direction === uml_element_port_1.Direction.Down &&
            target.direction === uml_element_port_1.Direction.Up &&
        ...
        /*
            ...
            #######
        */
        if (source.direction === uml_element_port_1.Direction.Up &&
            target.direction === uml_element_port_1.Direction.Down &&
...
function removeTransitNodes(path) {
    for (var i = 0; i < path.length - 2; i++) {
        ...
        if (isHorizontalLineSegment(p, q, r) || isVerticalLineSegment(p, q, r)) {
            ...
            var pathWithoutQ = tslib_1.__spread(pointsBeforeQ, pointsAfterQ);
...
function simplifyDeltas(deltas) {
    for (var i = 0; i < deltas.length - 3; i++) {
        ...
        if (d1.dy === 0 &&
            ...
            return simplifyDeltas(tslib_1.__spread(deltas.slice(0, i), [
                { dx: d1.dx + d3.dx, dy: 0 },
                { dx: 0, dy: d2.dy }
            ], deltas.slice(i + 3)));
        }
...
function determineCornerQueue(rect, edge, pointOnOuterEdge, destinationCorner) {
    ...
    var clockwisePathLength = computePathLength(tslib_1.__spread([pointOnOuterEdge], clockwiseCornerQueue));
    var counterClockwisePathLength = computePathLength(tslib_1.__spread([pointOnOuterEdge], counterClockwiseCornerQueue));
...
function lineSegmentsIntersect(p1, q1, p2, q2) {
    ...
    if (o1 === 0 /* Collinear */ && liesOnSegment(p1, p2, q1)) {
        return true;
    }
    // p1, q1 and p2 are collinear and q2 lies on segment p1q1
    if (o2 === 0 /* Collinear */ && liesOnSegment(p1, q2, q1)) {
        return true;
    }
    // p2, q2 and p1 are collinear and p1 lies on segment p2q2
    if (o3 === 0 /* Collinear */ && liesOnSegment(p2, p1, q2)) {
        return true;
    }
    // p2, q2 and q1 are collinear and q1 lies on segment p2q2
    if (o4 === 0 /* Collinear */ && liesOnSegment(p2, q1, q2)) {
        return true;
    }
...
function getOrientation(p, q, r) {
    ...
    return val > 0 ? 1 /* Clockwise */ : 2 /* CounterClockwise */;
}
/**
